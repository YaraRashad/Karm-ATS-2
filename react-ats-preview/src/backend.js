...
export function mapBackendData({ positions = [], candidates = [], applications = [], interviews = [], offers = [], scorecards = [], hiringRequests = [], audit = [], users = [] }) {
  ...
  const offerRows = offers.map(o => ({
    ...
    candidateName: fullName(o.application?.candidate) || "Candidate",
    candidateTitle: o.application?.candidate?.currentTitle || "",
    candidateEmail: isGeneratedCandidateEmail(o.application?.candidate?.email) ? "" : o.application?.candidate?.email || "",
    jobId: o.position?.id || o.positionId || o.application?.positionId || "",
    jobTitle: o.position?.title || "",
    jobDept: o.position?.department?.name || "",
    jobEntity: entityLabel[o.position?.entity] || o.position?.entity || "",
...
export const backendActions = {
  ...
  listThankYouLetters: () => api("/thank-you-letters"),
  confirmThankYouLetter: (id, token) => api(`/thank-you-letters/${id}/confirm`, { method: "POST", body: JSON.stringify({ token }) }),
