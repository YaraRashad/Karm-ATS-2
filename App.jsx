...
async function getPdfJsLib() {
  if (!cachedPdfJsLib) {
    const [{ default: pdfjsLib }, { default: workerSrc }] = await Promise.all([
      import("pdfjs-dist"),
  ...
  } else if (cachedPdfWorkerSrc && cachedPdfJsLib.GlobalWorkerOptions.workerSrc !== cachedPdfWorkerSrc) {
    cachedPdfJsLib.GlobalWorkerOptions.workerSrc = cachedPdfWorkerSrc;
  }
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-icon-danger { color: var(--red); background: var(--red-soft); border-color: rgba(220,38,38,0.22); }
  .btn-icon-danger:hover { color: var(--red); background: rgba(220,38,38,0.16); border-color: rgba(220,38,38,0.38); }
  .btn-icon-sm { width: 28px; height: 28px; padding: 0; border-radius: 6px; }
  .row-action-stack { display: flex; flex-direction: column; gap: 6px; align-items: stretch; width: 112px; }
  .row-action-stack .btn { width: 100%; min-height: 34px; justify-content: center; padding: 6px 10px; margin-left: 0 !important; white-space: normal; line-height: 1.15; }
  ...
  .toolbar-field .form-label { margin-bottom: 0; }
...
// ── HIRING REQUESTS ───────────────────────────────────────────────────────────
function HiringRequestsPage({ hiringRequests, setHiringRequests, currentRole, roleConfig, openModal, backendActions, reloadData }) {
  ...
  return (
    <>
      <div className="page-header">
        ...
        {canRequest && <button className="btn btn-primary" data-testid="open-add-hiring-request" onClick={() => openModal("addHiringRequest")}><Icon name="plus" size={14} /> Request New Hire</button>}
      </div>
      <div className="page-content" data-testid="page-requests">
          ...
          <div className="table-wrap">
            {hiringRequests.length === 0 ? (
              <div className="empty-state">
                <div className="empty-state-text">No hiring requests yet.</div>
              </div>
            ) : (
              <table>
...
function JobsPage({ jobs, setJobs, applications, candidates, roleConfig, canViewSalary, openModal, backendActions, reloadData, allUsers = [] }) {
  ...
  const recruiterOptions = Array.from(new Set(jobs.map(j => j.recruiter || "Unassigned").filter(Boolean))).sort();

  const filtered = jobs.filter(j =>
    (filterStatus === "All" || j.status === filterStatus) &&
    (filterEntity === "All" || j.entity === filterEntity) &&
    (filterDept === "All" || j.dept === filterDept) &&
    (filterRecruiter === "All" || (j.recruiter || "Unassigned") === filterRecruiter) &&
...
function CandidatesPage({ candidates, setCandidates, applications, jobs, roleConfig, openModal, backendActions, reloadData }) {
  ...
  const deleteCandidate = async (candidate) => {
    const appCount = applications.filter(a => a.candidateId === candidate.id && a.status === "Active").length;
    const okToDelete = window.confirm(`Delete "${candidate.name}"?\n\nThis will remove the candidate from active ATS views.${appCount ? `\n\nActive applications: ${appCount}` : ""}`);
    if (!okToDelete) return;
    try {
      if (backendActions?.deleteCandidate) {
      ...
      alert(e.message || "Could not delete this candidate.");
    }
  };
...
function SettingsPage({ currentRole, roleAssignments, setRoleAssignments, ROLES_CONFIG, auditLogs, backendUsers = [], backendActions, reloadData }) {
  ...
  const accessScopeOptions = [
    { value: "all_data", label: "All system data" },
    { value: "recruitment_data", label: "All recruitment data" },
    { value: "assigned_jobs", label: "Assigned jobs" },
    { value: "assigned_interviews", label: "Assigned interviews" },
  ];
  const roleDefaultScope = {
    admin: "all_data",
    recruiter: "recruitment_data",
    hiring_manager: "assigned_jobs",
    interviewer: "assigned_interviews",
  };
  ...
  const openEditUser = (user) => {
    ...
    setEditingUser(user);
    setEditUser({
      role: user.roleKey || ROLE_TO_BACKEND[user.role] || "recruiter",
      accessScope: user.accessScopeKey || roleDefaultScope[user.roleKey] || "recruitment_data",
  ...
  const updateEditUser = (field, value) => {
    setEditUser(prev => {
      ...
      if (field === "role") {
        next.accessScope = roleDefaultScope[value] || prev.accessScope;
        if (value === "admin") {
          next.canViewSalary = true;
          next.canApproveOffers = true;
          next.canApproveRequisitions = true;
        }
      }
      return next;
    });
  ...
  const submitEditUser = async () => {
    if (!editingUser?.id) {
      setEditUserError("This user can only be edited after it exists in the live backend.");
      return;
    }
    setEditUserError("");
    setSavingUser(true);
    try {
      await backendActions.updateUser(editingUser.id, {
        role: editUser.role,
        accessScope: editUser.accessScope,
        canViewSalary: !!editUser.canViewSalary,
        canApproveOffers: !!editUser.canApproveOffers,
        canApproveRequisitions: !!editUser.canApproveRequisitions,
        isActive: !!editUser.isActive,
      });
      await reloadData?.();
      setEditingUser(null);
    } catch (e) {
      setEditUserError(e.message || "Could not update this user.");
    } finally {
      setSavingUser(false);
    }
  };
      ...
      <div className="page-content">
        ...
        {activeTab === "users" && (
          <div>
                    ...
                    {userDirectory.map((u, idx) => {
                      ...
                      return (
                        <tr key={u.email || u.id}>
                          ...
                          <td>
                            <button className="btn btn-ghost btn-sm" onClick={() => openEditUser(u)}>
            ...
            {editingUser && (
              <div className="modal-overlay" onClick={() => !savingUser && setEditingUser(null)}>
                <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
                  <div className="modal-header">
                    <div>
                      <div className="modal-title">Edit User Access</div>
                      <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>{editingUser.fullName} · {editingUser.email}</div>
                    </div>
                  ...
                  <div className="modal-body">
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        <label className="form-label">Role</label>
                        <select className="form-select" value={editUser.role} onChange={e => updateEditUser("role", e.target.value)}>
                          {roleOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                        </select>
                      </div>
                      <div className="form-group">
                        <label className="form-label">Access scope</label>
                        <select className="form-select" value={editUser.accessScope} onChange={e => updateEditUser("accessScope", e.target.value)}>
                          {accessScopeOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                        </select>
                      </div>
                    </div>
                    <div style={{ display: "grid", gap: 10 }}>
                      <label className="chip" style={{ cursor: "pointer" }}><input type="checkbox" checked={editUser.isActive} onChange={e => updateEditUser("isActive", e.target.checked)} /> Active user</label>
                      <label className="chip" style={{ cursor: "pointer" }}><input type="checkbox" checked={editUser.canViewSalary} onChange={e => updateEditUser("canViewSalary", e.target.checked)} /> Can view salary details</label>
                      <label className="chip" style={{ cursor: "pointer" }}><input type="checkbox" checked={editUser.canApproveOffers} onChange={e => updateEditUser("canApproveOffers", e.target.checked)} /> Can approve offers</label>
                      <label className="chip" style={{ cursor: "pointer" }}><input type="checkbox" checked={editUser.canApproveRequisitions} onChange={e => updateEditUser("canApproveRequisitions", e.target.checked)} /> Can approve requisitions</label>
                    </div>
                  </div>
                  <div className="modal-footer">
                    <button className="btn btn-ghost" onClick={() => setEditingUser(null)} disabled={savingUser}>Cancel</button>
                    <button className="btn btn-primary" onClick={submitEditUser} disabled={savingUser}>{savingUser ? "Saving..." : "Save user access"}</button>
                  </div>
                </div>
              </div>
            )}
  ...
  const submit = async () => {
    ...
    try {
      if (ctx.backendActions?.createHiringRequest) {
        await ctx.backendActions.createHiringRequest(form);
        await ctx.reloadData?.();
      } else {
        const requestedByManager = ctx.currentRole === "Hiring Manager";
        const newRequest = {
          ...form,
          id: Date.now(),
          requestedBy: ctx.roleConfig.fullName,
          status: requestedByManager ? "Pending HR Approval" : "Pending Manager Approval",
  ...
  return (
    <div className="modal-overlay" onClick={closeModal}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={closeModal}>Cancel</button>
          <button className="btn btn-primary" data-testid="submit-add-hiring-request" onClick={submit} disabled={saving}>{saving ? "Submitting..." : "Submit Request"}</button>
        </div>
      </div>
    </div>
  );
